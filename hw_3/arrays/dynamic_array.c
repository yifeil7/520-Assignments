...
double DynamicArray_min(const DynamicArray *da) {
    ...
    double min_value = DynamicArray_get(da, 0);
    for (int i = 1; i < DynamicArray_size(da); i++) {
        double current_value = DynamicArray_get(da, i);
...
double DynamicArray_max(const DynamicArray *da) {
    ...
    double max_value = DynamicArray_get(da, 0);
    for (int i = 1; i < DynamicArray_size(da); i++) {
        double current_value = DynamicArray_get(da, i);
...
double DynamicArray_median(const DynamicArray *da) {
    ...
    DynamicArray *sorted_array = DynamicArray_copy(da);
    // Bubble sort (you can use a more efficient algorithm)
    for (int i = 0; i < DynamicArray_size(sorted_array) - 1; i++) {
        for (int j = 0; j < DynamicArray_size(sorted_array) - i - 1; j++) {
            if (DynamicArray_get(sorted_array, j) > DynamicArray_get(sorted_array, j + 1)) {
                // Swap elements
                double temp = DynamicArray_get(sorted_array, j);
                DynamicArray_set(sorted_array, j, DynamicArray_get(sorted_array, j + 1));
                DynamicArray_set(sorted_array, j + 1, temp);
            }
        }
    }

    double median;
    if (DynamicArray_size(sorted_array) % 2 == 0) {
        // Even number of elements, average the middle two
        int mid_index = DynamicArray_size(sorted_array) / 2;
        median = (DynamicArray_get(sorted_array, mid_index - 1) + DynamicArray_get(sorted_array, mid_index)) / 2.0;
    } else {
        // Odd number of elements, pick the middle one
        int mid_index = DynamicArray_size(sorted_array) / 2;
        median = DynamicArray_get(sorted_array, mid_index);
    }

    DynamicArray_destroy(sorted_array);
    return median;
}
...
DynamicArray *DynamicArray_range(double a, double b, double step) {
    assert(step > 0); // Ensure step is positive

    DynamicArray *result = DynamicArray_new();

    for (double value = a; value <= b; value += step) {
...
DynamicArray *DynamicArray_take(const DynamicArray *da, int n) {
    ...
    if (n >= 0) {
        ...
        // Take from the end
        start = DynamicArray_size(da) + n;
        end = DynamicArray_size(da);
    }
    ...
    // If not enough elements, set the rest to zero
